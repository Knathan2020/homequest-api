...
const supabase = process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_KEY
    ? (0, supabase_js_1.createClient)(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY)
    : null;
/**
...
router.post('/team/members/invite', async (req, res) => {
    try {
        const { teamId, email, fullName, role = 'member', department, phoneNumber, invitedBy } = req.body;
...
router.post('/:teamId/members', async (req, res) => {
    try {
        const { teamId } = req.params;
        const { name, phoneNumber, email, role, department, availability = 'available', expertise = [] } = req.body;
