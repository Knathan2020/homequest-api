...
const stripe_service_1 = __importStar(require("../services/stripe.service"));
const logger_1 = require("../utils/logger");
const logger = logger_1.loggers.system;
class PaymentController {
    ...
    static async getTiers(req, res) {
        try {
            const tiers = stripe_service_1.default.getTiers();
            res.json({
                success: true,
                data: tiers,
            });
    ...
    static async createCheckoutSession(req, res) {
        try {
            ...
            if (!stripe_service_1.PRICING_TIERS[tier]) {
                res.status(400).json({
                    success: false,
            ...
            const session = await stripe_service_1.default.createCheckoutSession({
                tier: tier,
                customerEmail: email,
                userId,
    ...
    static async getSubscription(req, res) {
        try {
            ...
            const subscription = await stripe_service_1.default.getSubscription(subscriptionId);
            res.json({
                success: true,
                data: {
                    id: subscription.id,
    ...
    static async cancelSubscription(req, res) {
        try {
            ...
            const subscription = await stripe_service_1.default.cancelSubscription(subscriptionId);
            res.json({
                success: true,
                data: {
                    id: subscription.id,
    ...
    static async updateSubscriptionTier(req, res) {
        try {
            ...
            if (!stripe_service_1.PRICING_TIERS[newTier]) {
                res.status(400).json({
                    success: false,
                    error: 'Invalid tier selected',
                });
            ...
            const subscription = await stripe_service_1.default.updateSubscriptionTier(subscriptionId, newTier);
            res.json({
                success: true,
                data: {
                    id: subscription.id,
     ...
     */
    static async checkProjectLimit(req, res) {
        try {
            ...
            const canAdd = stripe_service_1.default.canAddProject(tier, parseInt(currentProjectCount, 10));
            const limits = stripe_service_1.default.getTierLimits(tier);
            res.json({
                success: true,
                data: {
                    canAdd,
                    maxProjects: limits.maxProjects,
                    currentCount: parseInt(currentProjectCount, 10),
                },
            });
        }
        catch (error) {
     ...
     */
    static async checkUserLimit(req, res) {
        try {
            ...
            const canAdd = stripe_service_1.default.canAddUser(tier, parseInt(currentUserCount, 10));
            const limits = stripe_service_1.default.getTierLimits(tier);
            res.json({
                success: true,
                data: {
                    ...
                    currentCount: parseInt(currentUserCount, 10),
                },
            });
        }
        catch (error) {
